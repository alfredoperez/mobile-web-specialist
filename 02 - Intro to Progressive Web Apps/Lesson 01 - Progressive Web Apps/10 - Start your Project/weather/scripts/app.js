(function () {
  ...
  app.updateForecastCard = function (data) {
    ...
    // Verify data is newer that what we already have, if not, bail
  ...
  app.saveSelectedCities = function (city) {
    window.localforage.setItem('cities', app.selectedCities);
  };

  document.addEventListener('DOMContentLoaded', function () {
    window.localforage.getItem('cities',
      function (err, cities) {
        cities = cities === undefined || cities === null ? [] :
          cities;

        if (cities.length === 0)
          app.updateForecastCard(app.injectedForecast);
        else {
